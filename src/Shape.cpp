...
int Shape::Spawn(char ch, vector<vector<int>> zone){
    rotations = {};
    rotation = -1;
    type = ch;
...
int Shape::Rotate(vector<vector<int>> zone){
    rotation += 1;
...
bool Shape::Fall(vector<vector<int>>* zone){
    for (auto i : blocks){
...
int Shape::Left(vector<vector<int>> zone){
    for (auto i : blocks){
...
int Shape::Right(vector<vector<int>> zone){
    for (auto i : blocks){
        if (i.CheckDirection({1, 0}, zone)){
            return 0;
        }
...
int Shape::Stop(vector<vector<int>>* zone){
    for (int i = 0; i < blocks.size(); i++){
        zone->at(blocks[i].pos[1])[blocks[i].pos[0]] = 1;
    }
    return 0;
}
vector<vector<int>> Shape::Prediction(vector<vector<int>> zone){
...
int Shape::instantDescent(vector<vector<int>> zone,bool* isShapeSpawned){
